...
class CourseControllerTest {
    ...
    @Test
    void getShortCourse_Success() {
        // Arrange
        when(userService.validateSession(testSessionId)).thenReturn(testUser);
        when(courseService.getShortCourse("CS101")).thenReturn(Optional.of(testShortCourse));

        // Act
        ResponseEntity<CourseRepository.ShortCourse> response = courseController.getShortCourse("CS101", testSessionId);

        // Assert
        assertEquals(200, response.getStatusCodeValue());
        assertEquals("CS101", response.getBody().getCourseKey());
        assertEquals("Introduction to Computer Science", response.getBody().getTitle());
        verify(userService).validateSession(testSessionId);
        verify(courseService).getShortCourse("CS101");
    }

    @Test
    void getShortCourse_NotFound() {
        // Arrange
        when(userService.validateSession(testSessionId)).thenReturn(testUser);
        when(courseService.getShortCourse("CS999")).thenReturn(Optional.empty());

        // Act & Assert
        assertThrows(ResourceNotFoundException.class, () ->
                courseController.getShortCourse("CS999", testSessionId)
        );
        verify(userService).validateSession(testSessionId);
        verify(courseService).getShortCourse("CS999");
    }
    ...
    void searchCourses_Success() {
        ...
        List<Course> courses = Arrays.asList(testCourse);
        when(userService.validateSession(testSessionId)).thenReturn(testUser);
        when(courseService.searchCoursesByRegex("CS")).thenReturn(courses);
    ...
    @Test
    void searchShortCourses_Success() {
        // Arrange
        List<CourseRepository.ShortCourse> shortCourses = Arrays.asList(testShortCourse);
        when(userService.validateSession(testSessionId)).thenReturn(testUser);
        when(courseService.searchShortCoursesByRegex("CS")).thenReturn(shortCourses);
